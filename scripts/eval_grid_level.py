...
def reconstruct_forced_level(
    ...
    return pred, ref, lod_int, lod_w, lod_h
...
def evaluate_lod(
    ...
    pred, ref, lod_int, lod_w, lod_h = reconstruct_forced_level(
        model=model,
        dataset=dataset,
        level=level,
        lod_value=lod_value,
    ...
    for tex_type in dataset.available_textures:
        ...
        if save_images:
            save_texture_image(lod_dir / f"{tex_type}.png", tex_type, pred_tex)
...
def evaluate_boundary(
    ...
    pred_left, ref, lod_int, lod_w, lod_h = reconstruct_forced_level(
        model=model,
        dataset=dataset,
        level=left_level,
        lod_value=lod_value,
    ...
    pred_right, _, _, _, _ = reconstruct_forced_level(
        model=model,
        dataset=dataset,
        level=right_level,
        lod_value=lod_value,
    ...
    rows: list[dict[str, str]] = []
    boundary_dir = output_dir / f"boundary_L{left_level}_L{right_level}_lod_{lod_int:02d}"
    if save_images:
        boundary_dir.mkdir(parents=True, exist_ok=True)

    print(f"\nBoundary L{left_level}/L{right_level}, LOD {lod_int}, resolution {lod_w}x{lod_h}")
    for tex_type in dataset.available_textures:
        cn_start, cn_end = CANONICAL_CHANNEL_SLICES[tex_type]
        left_tex = pred_left[:, :, cn_start:cn_end]
        right_tex = pred_right[:, :, cn_start:cn_end]
        ref_tex = ref[:, :, cn_start:cn_end]

        left_mse = ((left_tex - ref_tex) ** 2).mean().item()
        right_mse = ((right_tex - ref_tex) ** 2).mean().item()
        delta_mse = ((left_tex - right_tex) ** 2).mean().item()
        print(
            f"  {tex_type:12s} "
            f"left_vs_gt={psnr_from_mse(left_mse):6.2f} dB  "
            f"right_vs_gt={psnr_from_mse(right_mse):6.2f} dB  "
            f"left_vs_right={psnr_from_mse(delta_mse):6.2f} dB"
        )
        rows.append(
            {
                ...
                "lod": str(lod_int),
                "texture": tex_type,
                "left_mse": f"{left_mse:.8f}",
                "left_psnr_db": f"{psnr_from_mse(left_mse):.4f}",
                "right_mse": f"{right_mse:.8f}",
                "right_psnr_db": f"{psnr_from_mse(right_mse):.4f}",
                "left_right_mse": f"{delta_mse:.8f}",
                "left_right_psnr_db": f"{psnr_from_mse(delta_mse):.4f}",
                "width": str(lod_w),
                "height": str(lod_h),
            }
        )
        ...
        if save_images:
            save_texture_image(boundary_dir / f"{tex_type}_left_L{left_level}.png", tex_type, left_tex)
            save_texture_image(boundary_dir / f"{tex_type}_right_L{right_level}.png", tex_type, right_tex)
            diff = (left_tex - right_tex).abs()
            diff = diff / max(float(diff.max().item()), 1e-6)
            save_texture_image(boundary_dir / f"{tex_type}_absdiff.png", tex_type, diff)
...
def main() -> None:
    ...
    parser.add_argument("--boundary", action="store_true", help="Compare adjacent grid levels at their shared LOD boundaries")
    parser.add_argument(
        "--boundaries",
        default=None,
        help="Comma-separated boundary specs like 0:1:5,1:2:7. Defaults to all adjacent level boundaries.",
    )
    ...
    if args.boundary:
        if args.boundaries:
            boundary_specs = []
            for raw_spec in args.boundaries.split(","):
                raw_spec = raw_spec.strip()
                if not raw_spec:
                    continue
                left, right, lod = raw_spec.split(":")
                boundary_specs.append((int(left), int(right), float(lod)))
        else:
            boundary_specs = []
            for left_level in range(len(model.level_mip_ranges) - 1):
                right_level = left_level + 1
                boundary_lod = float(model.level_mip_ranges[right_level][0])
                boundary_specs.append((left_level, right_level, boundary_lod))

        boundary_rows: list[dict[str, str]] = []
        for left_level, right_level, lod_value in boundary_specs:
            if left_level < 0 or right_level >= len(model.grid_configs) or left_level >= right_level:
                raise ValueError(f"Invalid boundary spec {left_level}:{right_level}:{lod_value}")
        ...
        boundary_metrics_path = output_dir / "boundary_metrics.csv"
        with boundary_metrics_path.open("w", newline="", encoding="utf-8") as f:
        ...
        print(f"\nWrote boundary metrics: {boundary_metrics_path}")
        return
